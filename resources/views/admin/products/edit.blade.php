        ...
        <div class="white-box">
          ...
          <form action="{{ route('admin.products.update', ['id'=>$product->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group" id="prod-attributes">
              <h6>Attributes</h6>
              @php $all_variants = []; @endphp
              @foreach($variants as $key => $val)
                
                @if($val[0]['variant_option'])
                  @foreach($val[0]['variant_option'] as $item)
                    @php
                    $productt_attribute = App\ProductAttributeOption::find($item) ? App\ProductAttributeOption::find($item)->option : '';
                    $all_variants[] = $productt_attribute;
                    @endphp
              ...
              @endforeach
              <input type="hidden" id="hidden_product_attributes" value="{{json_encode($all_variants)}}">
              @foreach ($product_variants as $variant)
                <label for="" class="label-inline-block">Select {{$variant->attribute}}</label>
                @foreach ($variant->options as $option)
                    <input type="checkbox" name="variant_{{$variant->attribute}}[]" id="{{$option->option}}" value="{{$option->id}}">
                    <label for="{{$option->option}}">{{ucfirst($option->option)}}</label> &nbsp;
                @endforeach
