					...
					<form action="{{ route('cart.add') }}" method="post" style="width:100%" class="add-to-cart-form">
							...
							<div class="form-group col-12">
								@if($variant['variant_option'] != null)
									...
									@foreach ($variant['variant_option'] as $item)
										...
										@if($opt)
										<input type="radio" {{$checked}} name="variant_{{$variant['variant_name']}}" class="product-attribute-option-input" id="v-{{$item}}" value="{{$opt->option}}" required>
						...
						<div class="product_btns">
							<div class="product_btn_itm">
								...
								<button type="submit" data-toggle="tooltip" title="{{$product_variants ? 'Please choose a product variant': ''}}" data-placement="top" class="add_to_cart">Add to Cart</button>
