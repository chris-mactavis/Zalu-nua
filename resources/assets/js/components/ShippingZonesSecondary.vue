<template>
  <div class="white-box">
        ...
        <div class="col-sm-4">
          <div class="zone-box">
            <div class="zone" v-for="(country, inc) in allCountries">
              ...
              >
              <label :for="'zoneCountry'+inc">
                {{ country.country_name }}
                <span
                  class="badge badge-success"
                  v-if="country.zone"
                >{{country.zone.zone.zone}}</span>
              </label>
            </div>
          </div>
          <button
          ...
          <div class="zone-box">
            <div class="zone" v-for="(country, inc) in markAsSelected">
              <input
                ...
                :country="country"
              >
              <label :for="'selectedZoneCountry'+inc">{{ country }}</label>
            </div>
          </div>
          <button
            id="add-to-zone-country"
            class="btn btn-sm btn-primary add-to-zone"
            data-toggle="modal"
            data-target="#add-state-to-zone-country"
            :disabled="markSelectedCountries.length < 1"
          >Add to Zone</button>
          <button
    ...
    <div class="modal fade" id="add-state-to-zone-country" tabindex="-1" role="dialog">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">Add Countries to Zone</h4>
          </div>
...
export default {
  ...
  methods: {
    ...
    moveToChosen() {
      this.markAsSelected = this.selectedCountries;
      Array.from(
        document.getElementsByClassName("zone-selector-country")
      ).forEach(x => (x.checked = false));
    },
    selectAllItems() {
      Array.from(
        document.getElementsByClassName("zone-selector-country-2")
      ).forEach(x => (x.checked = true));

      this.markSelectedCountries = this.selectedCountries.map(x => {
        let rObj = {};
        rObj = x;
        return rObj;
      });
    ...
    deleteSelected() {
      Array.from(
        ...
        if (x.checked) {
          let country = x.getAttribute("country");
          this.markSelectedContries = this.markSelectedCountries.filter(
            x => x != country
          );
    ...
    addToZone() {
      this.loading = true;
      this.selected_zone.countries = this.markSelectedCountries;
      axios
        .post(`/admin/add-countries-to-shipping-zones`, this.selected_zone)
        .then(response => {
          this.loading = false;
