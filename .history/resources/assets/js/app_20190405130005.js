...
$('#add-attribute-submit').on('click', function (evt) {
    let var_image = $('#var_image');
    let attribute_select = $('#attribute-select');
    let all_attributes = $('#all-attributes');
    if (var_image.val() && attribute_select.val()) {
        evt.preventDefault();
        all_attributes.val().push({image: var_image.val(), attributes: attribute_select.val()})
    }
})