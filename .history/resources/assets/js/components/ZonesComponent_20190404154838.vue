        ...
        <table class="table table-striped">
          ...
          <tbody>
            <tr v-for="(zone, index) in zones" :key="index">
              <td>{{zone[0].zone}}</td>
              <td>{{zone[0].rate | formatNumber}}</td>
              <td class="well">
                <span class="badge badge-info" v-for="(zn, index) in zone" :key="index" v-if="zone">{{zn.state_name ? zn.state_name : zn.country_name }}</span>
              </td>
              <td class="text-center">
                <div class="edit-btn-wrap">
                  ...
                  <div class="edit-btn">
                    <a href="#" title="delete" @click="deleteZone(zone[0])">
...
export default {
  ...
  methods: {
    ...
    editZone(index) {
      this.edit_zone = index;
    },

    updateZone() {
      this.loading = true;
      axios
        .patch(`/admin/shipping-zones/${this.edit_zone.id}`, this.edit_zone)
        .then(response => {
          this.loading = false;
          this.edit_zone = {};
          $(".close").click();
    ...
    deleteZone(index) {
      ...
      if (confirm("Are you sure you want to delete this zone?")) {
        axios.delete(`/admin/shipping-zones/${index.id}`).then(response => {
          location.reload();
        });
      }
    }
