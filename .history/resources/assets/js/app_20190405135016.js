...
$('#add-attribute-btn').on('click', function (evt) {
    ...
    $single_attribute = `<div id="single-attribute">
              ...
              <div class="form-group">
                ...
                <select name="product_attributes[]" multiple required class="form-control input-lg" id="attribute-select-${iterate + 1}">
    ...
    product_attributes.forEach(x => {
        $('#attribute-select-'+iterate + ).append(`<option value="${x.id}">${x.option}</option>`)
    })
    $('#iterate').val(++iterate);
})