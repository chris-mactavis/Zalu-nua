...
class AdminProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->selected_attributes as $attribute) {
            return $query = ProductVariant::whereProductAttributeOptionId($attribute);
            $product_attribute_id = ProductAttributeOption::find($attribute)->
            if ($query->exists()) {
                $query->update(['product_attribute_option_id' => $attribute, 'product_attribute_id' => $product_attribute_id, 'product_id' => $product->id]);
            }
        }
        ...
        //     $product_image = $request->file('var_image_' . ($i + 1))[0];
        //     $product_image_new_name = time() . $product_image->getClientOriginalName();
        //     $product_image->move('uploads/banners', $product_image_new_name);

        //     $product->product_attributes()->create([
        //         'product_attributes' => $options,
        //         'image' => 'uploads/banners/' . $product_image_new_name
        //     ]);
        // }
