...
class ShippingZonesController extends Controller
{
    ...
    {
        return $zones = Zone::leftJoin('shipping_zones as sz', 'sz.zone_id', 'zones.id')
            ->leftJoin('states as s', 's.id', 'sz.state_id')
            ->leftJoin('cities as cts', 's.id', 'cts.state_code')
            ->leftJoin('countries as c', 'c.id', 'sz.country_id')
            ->select('zones.*', 's.state_name', 'c.country_name', 'cts.city')
            ->orderBy('zones.zone')
            ->get();
