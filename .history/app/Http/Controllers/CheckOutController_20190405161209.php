...
class CheckOutController extends Controller
{
    ...
    public function getDeliveryRate($state_id, $country_id = null)
    {
        if($state_id) {
            $delivery_rate = ShippingZone::whereStateId($state_id)->first();
            if($delivery_rate) {
                return $delivery_rate->zone()->first()->rate;
            } else {
                return 2500;
            }
        } else {
            $delivery_rate = ShippingZone::whereCountryId($country_id)->first();
            if ($delivery_rate) {
                return $delivery_rate->zone()->first()->rate;
            } else {
                return 2500;
    ...
    public function docheckout(Request $request)
    {
        ...
        if ($delivery_address == 'new') {
            ...
            $userinfo = Userinfo::create([
                ...
                'state_id' => $state_id,
                'phone' => $phone,
                'country' => Country::find($request->country_id)->country_name
            ]);
        ...
        if ($new_userinfo->country == 'Nigeria') {
            return $state_id = $userinfo->state_id;
            $delivery_rate = $this->getDeliveryRate($state_id);
        } else {
            $country_id = $request->country_id;
            return $delivery_rate = $this->getDeliveryRate(null, $country_id);
        }
